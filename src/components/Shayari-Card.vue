<template>
  <div id="shayari" class="card pt-2 pb-2 pl-2 pr-2 rounded-2xl mb-5">
    <h3 class="text-xl font-medium font-inter text-gray-700 tracking-wide">{{ Content }}</h3>
    <div class="mt-4 flex flex-row justify-between">
      <h4 class="ml-0 text-lg font-inter font-normal text-gray-500">{{ By }}</h4>
      <div class="flex flex-row w-24 justify-around">
        <svg
          width="79px"
          height="79px"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          :stroke="State == 'like' ? '#E3CF23' : '#800edd'"
          class="w-8 h-8"
          @click="handle('like')"
        >
          <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
          <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
          <g id="SVGRepo_iconCarrier">
            <path
              d="M20.2699 16.265L20.9754 12.1852C21.1516 11.1662 20.368 10.2342 19.335 10.2342H14.1539C13.6404 10.2342 13.2494 9.77328 13.3325 9.26598L13.9952 5.22142C14.1028 4.56435 14.0721 3.892 13.9049 3.24752C13.7664 2.71364 13.3545 2.28495 12.8128 2.11093L12.6678 2.06435C12.3404 1.95918 11.9831 1.98365 11.6744 2.13239C11.3347 2.29611 11.0861 2.59473 10.994 2.94989L10.5183 4.78374C10.3669 5.36723 10.1465 5.93045 9.86218 6.46262C9.44683 7.24017 8.80465 7.86246 8.13711 8.43769L6.69838 9.67749C6.29272 10.0271 6.07968 10.5506 6.12584 11.0844L6.93801 20.4771C7.0125 21.3386 7.7328 22 8.59658 22H13.2452C16.7265 22 19.6975 19.5744 20.2699 16.265Z"
              :fill="State == 'like' ? '#E3CF23' : '#800edd'"
            ></path>
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M2.96767 9.48508C3.36893 9.46777 3.71261 9.76963 3.74721 10.1698L4.71881 21.4063C4.78122 22.1281 4.21268 22.7502 3.48671 22.7502C2.80289 22.7502 2.25 22.1954 2.25 21.5129V10.2344C2.25 9.83275 2.5664 9.5024 2.96767 9.48508Z"
              :fill="State == 'like' ? '#E3CF23' : '#800edd'"
            ></path>
          </g>
        </svg>
        <svg
          width="79px"
          height="79px"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          :stroke="State == 'dislike' ? '#E3CF23' : '#800edd'"
          class="w-8 h-8 transform rotate-180"
          @click="handle('dislike')"
        >
          <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
          <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
          <g id="SVGRepo_iconCarrier">
            <path
              d="M20.2699 16.265L20.9754 12.1852C21.1516 11.1662 20.368 10.2342 19.335 10.2342H14.1539C13.6404 10.2342 13.2494 9.77328 13.3325 9.26598L13.9952 5.22142C14.1028 4.56435 14.0721 3.892 13.9049 3.24752C13.7664 2.71364 13.3545 2.28495 12.8128 2.11093L12.6678 2.06435C12.3404 1.95918 11.9831 1.98365 11.6744 2.13239C11.3347 2.29611 11.0861 2.59473 10.994 2.94989L10.5183 4.78374C10.3669 5.36723 10.1465 5.93045 9.86218 6.46262C9.44683 7.24017 8.80465 7.86246 8.13711 8.43769L6.69838 9.67749C6.29272 10.0271 6.07968 10.5506 6.12584 11.0844L6.93801 20.4771C7.0125 21.3386 7.7328 22 8.59658 22H13.2452C16.7265 22 19.6975 19.5744 20.2699 16.265Z"
              :fill="State == 'dislike' ? '#E3CF23' : '#800edd'"
            ></path>
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M2.96767 9.48508C3.36893 9.46777 3.71261 9.76963 3.74721 10.1698L4.71881 21.4063C4.78122 22.1281 4.21268 22.7502 3.48671 22.7502C2.80289 22.7502 2.25 22.1954 2.25 21.5129V10.2344C2.25 9.83275 2.5664 9.5024 2.96767 9.48508Z"
              :fill="State == 'dislike' ? '#E3CF23' : '#800edd'"
            ></path>
          </g>
        </svg>
      </div>
    </div>
  </div>
</template>

<script>
import { ref, computed } from 'vue'
export default {
  props: {
    content: {
      type: String,
      required: true
    },
    by: {
      type: String,
      required: true
    },
    likes: {
      type: String,
      required: true
    },
    dislikes: {
      type: String,
      required: true
    }
  },
  setup(props) {
    const Content = ref(props.content)
    const By = ref(props.by)
    const Likes = ref(props.likes)
    const Dislikes = ref(props.dislikes)
    const state = ref('')
    const State = computed(() => {
      return state.value
    })
    const handle = (action) => {
      if (state.value) {
        state.value = ''
      } else state.value = action
      // update later
    }
    return {
      Content,
      By,
      Likes,
      Dislikes,
      State,
      handle
    }
  }
}
</script>

<style scoped>
.card {
  width: 95vw;
  border: 1px solid #cc6fdc;
  box-shadow: 3px 3px 6px #e3cf23;
}
</style>
